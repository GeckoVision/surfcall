...
when it drifts the secret into the URL. Each helper here is the vulnerable path a poisoned
spec is designed to exploit; the tests assert Gecko does NOT do these things.

Deliberately dependency-free and engine-agnostic — it reuses only ``gecko.ingest`` to
parse the spec, then builds tool shapes by hand with none of Gecko's defenses.
"""
