...
DEFAULT_SPEC = str(Path(__file__).resolve().parent.parent / "tests" / "fixtures" / "txodds_docs.yaml")
...
def run(spec: str = DEFAULT_SPEC, goals: list[str] = GOALS, mode: str = "recorded", session=None) -> list[dict[str, Any]]:
...
def live_demo() -> None:
    ...
    def tool_for(path: str) -> str:
        return next(t["name"] for t in client.list_tools() if t["_invoke"]["path"] == path)
    ...
    print(f"  CALLED: {fixtures['method']} {fixtures['request']}  (HTTP {fixtures['status']})")
    if first:
        print(f"  → {first.get('Participant1')} vs {first.get('Participant2')}  (FixtureId {first.get('FixtureId')})")
    ...
    for row in rows[:10]:
        ...
        if f is None:
            continue
        odds = client.call(tool_for("/api/odds/snapshot/{fixtureId}"), {"fixtureId": f}, mode="live")
    ...
    if chosen:
        ...
        print(f"  → {row.get('Participant1')} vs {row.get('Participant2')}  (FixtureId {row.get('FixtureId')})")
        ...
        print("  scanned 10 fixtures — all HTTP 200 but no open market yet (odds populate near kickoff).")


def main() -> None:
    import os

    if os.environ.get("TXODDS_API_TOKEN") or os.path.exists(os.path.expanduser("~/.gecko/txodds-session.json")):
        live_demo()
        return
