...
then activate → apiToken. The Python access layer (`surfcall/access.py`) already
...
3. **Activate**: the same script (or `surfcall/access.py::establish_session`) does guest-JWT → sign `txSig:leagues:jwt` → `/api/token/activate` → **apiToken**.
4. **Flip surfcall to live**: construct a `Session(jwt=<jwt>, api_token=<apiToken>)` and pass it to `AgentApiClient(spec, session=...)`, then call with `mode="live"`. The demo's recorded data becomes real World Cup data — same code path.
